...
vector<vector<double>> PathPlanner::generate_trajectory(Car &car){
    ...
    // Initial conditions
    vector<double> start {0., ref_vel/2.24, 0.};
    ...
    // Correction due to time discretization
    double N_double = (target_dist / (.02*ref_vel/2.24));
    ...
    // Final conditions
    vector<double> end {target_dist, ref_vel / 2.24, 0.};

    // Jerk minimizing trajectory
    double T = (target_dist / ref_vel * 2.24); // good only for constant speed
    ...
    for(int i=1; i <= 50-car.previous_path_size; ++i)
    {
        ...
        double x_point = polyeval(coeffs, t_add_on) * cos(angle);
